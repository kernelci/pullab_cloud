...
# Subpath under shared/kernel-rpms/ to pull kernel RPMs from. Binary tests use
# the per-arch binary dir (default); source-RPM tests (simple-source-reboot)
# override this to "src" before calling list_kernels_from_s3/download_kernel_rpm.
KERNEL_RPM_SUBPATH="${KERNEL_RPM_SUBPATH:-binary/${ARCH}}"
...
{
    S3_PATH="s3://${RESULTS_BUCKET}/${RUN_PREFIX}/shared/kernel-rpms/${KERNEL_RPM_SUBPATH}/"
...
install_test_dependencies()
{
    ...
    if [ -f "$deps_file" ]; then
        ...
        # No dependencies.txt: fall back to the historical minimal build set so
        # out-of-tree tests that relied on the old behaviour keep working.
        echo "WARNING: dependencies.txt not found; installing default gcc make tar" >&2
        for pkg in gcc make tar; do
